...
let rec forSignatureTypeItem = (env, exported: SharedTypes.Module.exported, item) => {
  ...
  | Sig_value(ident, {val_type, val_attributes, val_loc: loc}
    ...
    let declared = addItem(
      ...
      ~extent=loc,
      ~stamp=Current.ident_binding_time_408(ident),
      ~env,
      ~contents,
      val_attributes,
      exported.values,
  ...
  | Sig_type(
      ...
      kind: switch type_kind {
        ...
        | Type_variant(constructors) => {
          Variant(constructors |. Belt.List.map(({cd_loc, cd_id, cd_args, cd_res, cd_attributes}) => {
            ...
            let stamp = Current.ident_binding_time_408(cd_id);
            let contents = {
        ...
        | Type_record(labels, _) => Record(labels |> List.map(
          ({ld_id, ld_type}) => {
            let astamp = Current.ident_binding_time_408(ld_id);
    ...
    }, ~name=Location.mknoloc(Ident.name(ident)), ~stamp=Current.ident_binding_time_408(ident), ~env, type_attributes, exported.types, env.stamps.types);
...
let forTypeDeclaration = (~env, ~exported: Module.exported, {typ_id, typ_loc, typ_params, typ_name: name, typ_attributes, typ_type, typ_kind, typ_manifest}) => {
  let stamp = Current.ident_binding_time_408(typ_id);
...
let forSignatureItem = (~env, ~exported: Module.exported, item) => {
  ...
  | Tsig_value({val_id, val_loc, val_name: name, val_desc, val_attributes}) => {
    let declared = addItem(
      ~name,
      ~stamp=Current.ident_binding_time_408(val_id),
      ~extent=val_loc,
  ...
  | Tsig_module({md_id, md_attributes, md_loc, md_name: name, md_type: {mty_type}}) => {
    ...
    let declared = addItem(~contents, ~name, ~extent=md_loc, ~stamp=Current.ident_binding_time_408(md_id), ~env, md_attributes, exported.modules, env.stamps.modules);
    [{...declared, contents: Module.Module(declared.contents)}]
  }
  ...
  optMap(({vb_loc, vb_pat: {pat_desc, pat_type}, vb_attributes}) =>
    ...
    switch (pat_desc) {
      | Tpat_var(ident, name) =>
        ...
        let declared = addItem(~name, ~stamp=Current.ident_binding_time_408(ident), ~env, ~extent=vb_loc, ~contents, vb_attributes, exported.values, env.stamps.values);
...
| Tstr_module({mb_id, mb_attributes, mb_loc, mb_name: name, mb_expr: {mod_desc}}) => {
  ...
  let declared = addItem(~contents, ~name, ~extent=mb_loc, ~stamp=Current.ident_binding_time_408(mb_id), ~env, mb_attributes, exported.modules, env.stamps.modules);
  [{...declared, contents: Module.Module(declared.contents)}]
}
...
| Tstr_primitive({val_id, val_name: name, val_loc, val_attributes, val_val: {val_type}}) => {
  let declared = addItem(~extent=val_loc, ~contents={Value.recursive: false, typ: Shared.makeFlexible(val_type)}, ~name, ~stamp=Current.ident_binding_time_408(val_id), ~env, val_attributes, exported.values, env.stamps.values);
  [{...declared, contents: Module.Value(declared.contents)}]
}
| Tstr_type(_, decls) =>
