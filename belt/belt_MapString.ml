...
let rec set t (newK : key) (newD : _) =
  ...
  | Some n ->
      ...
      then N.return (N.updateValue n newD)
        ...
        (let v = N.value n in
         if newK < k
         then N.bal (set (N.left n) newK newD) k v (N.right n)
         else N.bal (N.left n) k v (set (N.right n) newK newD))
let rec updateU t (x : key) f =
  match N.toOpt t with
  | None  ->
      (match f None with | None  -> t | Some data -> N.singleton x data)
  | Some n ->
      let k = N.key n in
      if x = k
      then
        ...
        (let (l,r,v) = ((N.left n), (N.right n), (N.value n)) in
         if x < k
         then
           let ll = updateU l x f in (if l == ll then t else N.bal ll k v r)
         else
           (let rr = updateU r x f in if r == rr then t else N.bal l k v rr))
    ...
    then
      (match N.toOpt l with
       ...
       | Some left ->
           ...
           if ll == l then N.return n else let open N in bal ll v (value n) r)
    else
      (match N.toOpt r with
       | None  -> N.return n
       | Some right -> let rr = removeAux right x in N.bal l v (N.value n) rr)
let remove n x =
...
let mergeMany h arr =
  ...
  let v = ref h in
  for i = 0 to len - 1 do
    (let (key,value) = A.getUnsafe arr i in v := (set (!v) key value))
  done;
  !v
