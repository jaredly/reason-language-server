...
include
  (struct
     ...
     let dataSet: t -> I.t -> unit = fun o  -> fun v  -> o.data <- v
  ...
  then
    ...
    then
      (match N.toOpt (N.left nt) with
       | None  -> N.return nt
       | Some l -> (N.leftSet nt (remove0 l x); N.return (N.balMutate nt)))
    else
      (match N.toOpt (N.right nt) with
       | None  -> N.return nt
       | Some r -> (N.rightSet nt (remove0 r x); N.return (N.balMutate nt)))
let remove d v =
...
let removeMany (d : t) xs =
  ...
  match N.toOpt oldRoot with
  | None  -> ()
  | Some nt -> let len = A.length xs in dataSet d (removeMany0 nt xs 0 len)
...
let removeCheck (d : t) v =
  ...
  | Some oldRoot2 ->
      ...
      (if newRoot != oldRoot then dataSet d newRoot; !removed)
let rec addCheck0 t (x : value) added =
  match N.toOpt t with
  | None  -> (added := true; N.singleton x)
  | Some nt ->
      let k = N.value nt in
      if x = k
      then t
        ...
        (let (l,r) = let open N in ((left nt), (right nt)) in
         if x < k
         then (let ll = addCheck0 l x added in N.leftSet nt ll)
...
let addCheck (m : t) e =
  ...
  if newRoot != oldRoot then dataSet m newRoot; !added
let add d k =
  let oldRoot = data d in
  let v = I.addMutate oldRoot k in if v != oldRoot then dataSet d v
...
let reduce d acc cb = reduceU d acc (fun a  -> fun b  -> cb a b)
...
let split d key =
  ...
  then
    ...
    (((t ~data:(N.fromSortedArrayAux arr 0 next)),
       (t ~data:(N.fromSortedArrayAux arr next (len - next)))), false)
  else
    (((t ~data:(N.fromSortedArrayAux arr 0 i)),
       (t ~data:(N.fromSortedArrayAux arr (i + 1) ((len - i) - 1)))), true)
let keepU d p = t ~data:(N.keepCopyU (data d) p)
let keep d p = keepU d (fun a  -> p a)
let partitionU d p =
  let (a,b) = N.partitionCopyU (data d) p in ((t ~data:a), (t ~data:b))
  ...
  | (Some dataa0,Some datab0) ->
      ...
      (ignore @@ (N.fillArray dataa0 0 tmp);
       ...
       then make ()
         ...
         (let tmp2 =
            A.makeUninitializedUnsafe (min sizea sizeb) (N.value dataa0) in
       ...
       (ignore @@ (N.fillArray dataa0 0 tmp);
        ...
        then t ~data:(N.copy dataa)
          ...
          (let tmp2 = A.makeUninitializedUnsafe sizea (N.value dataa0) in
           ...
           t ~data:(N.fromSortedArrayAux tmp2 0 k))) : t)
let union (dataa : t) (datab : t) =
  (let (dataa,datab) = ((data dataa), (data datab)) in
   match ((N.toOpt dataa), (N.toOpt datab)) with
   ...
   | (Some dataa0,Some datab0) ->
       ...
       (ignore @@ (N.fillArray dataa0 0 tmp);
        ...
        then t ~data:(N.fromSortedArrayAux tmp 0 totalSize)
        else
