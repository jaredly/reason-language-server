  ...
  then
    ...
    then
      (match N.toOpt (N.left nt) with
       | None  -> N.return nt
       | Some l ->
           (N.leftSet nt (remove0 ~cmp l x); N.return (N.balMutate nt)))
    else
      (match N.toOpt (N.right nt) with
       | None  -> N.return nt
       | Some r ->
           (N.rightSet nt (remove0 ~cmp r x); N.return (N.balMutate nt)))
let remove d v =
  ...
  | Some oldRoot2 ->
      let newRoot =
        remove0 ~cmp:(Belt_Id.getCmpInternal (S.cmp d)) oldRoot2 v in
...
let removeCheck d v =
  ...
  | Some oldRoot2 ->
      ...
      (if newRoot != oldRoot then S.dataSet d newRoot; !removed)
let rec addCheck0 t x added ~cmp  =
  ...
  | Some nt ->
      ...
      then t
        ...
        (let (l,r) = let open N in ((left nt), (right nt)) in
         if c < 0
         then (let ll = addCheck0 ~cmp l x added in N.leftSet nt ll)
...
let addCheck m e =
  ...
  let newRoot =
    addCheck0 ~cmp:(Belt_Id.getCmpInternal (S.cmp m)) oldRoot e added in
  if newRoot != oldRoot then S.dataSet m newRoot; !added
let add m e =
  let oldRoot = S.data m in
...
let split d key =
  ...
  then
    ...
    (((S.t ~data:(N.fromSortedArrayAux arr 0 next) ~cmp),
       (S.t ~data:(N.fromSortedArrayAux arr next (len - next)) ~cmp)), false)
  else
        ...
        (let p = Belt_Id.getCmpInternal cmp in
         ...
         then S.t ~cmp ~data:N.empty
           ...
           (let tmp2 =
            ...
            S.t ~data:(N.fromSortedArrayAux tmp2 0 k) ~cmp))) : _ t)
let diff a b =
  (let cmp = S.cmp a in
