...
module Version1 =
  struct
    ...
    and 'source _SharedTypes__SimpleType__expr =
      ...
      | Variable of string 
      | AnonVariable 
      | RowVariant of (string * 'source _SharedTypes__SimpleType__expr
      option) list * bool 
    ...
    and deserialize_Asttypes____loc : type arg0.
      ...
      fun aTransformer ->
        fun record ->
          ...
          | ((Json.Object (items))[@explicit_arity ]) ->
              let inner attr_loc =
                let inner attr_txt =
                  Belt.Result.Ok { txt = attr_txt; loc = attr_loc } in
          ...
          | _ -> ((Belt.Result.Error (["Expected an object"]))
              [@explicit_arity ])
    and deserialize_Belt__Belt_HashMapInt____t : type arg0.
      (Json.t -> (arg0, string list) Belt.Result.t) ->
        Json.t -> (arg0 _Belt__Belt_HashMapInt__t, string list) Belt.Result.t
      =
      fun arg0Transformer ->
        ...
        | ((Json.Object (items))[@explicit_arity ]) ->
            let inner attr_loc_ghost =
              let inner attr_loc_end =
                ...
                match Belt.List.getAssoc items "loc_start" (=) with
                | None -> ((Belt.Result.Error (["No attribute loc_start"]))
                    [@explicit_arity ])
                | ((Some (json))[@explicit_arity ]) ->
                    (match deserialize_Stdlib__lexing____position json with
                     | ((Belt.Result.Error (error))[@explicit_arity ]) ->
                         ((Belt.Result.Error
                             (("attribute loc_start" :: error)))
                         [@explicit_arity ])
                     | ((Belt.Result.Ok (data))[@explicit_arity ]) ->
                         inner data) in
             ...
             | ((Some (json))[@explicit_arity ]) ->
                 (match (fun bool ->
                  ...
                  | ((Belt.Result.Error (error))[@explicit_arity ]) ->
                      ...
                      [@explicit_arity ])
                  | ((Belt.Result.Ok (data))[@explicit_arity ]) -> inner data))
        | _ -> ((Belt.Result.Error (["Expected an object"]))
            [@explicit_arity ])
    ...
    and deserialize_SharedTypes__SimpleType__body : type arg0.
      (Json.t -> (arg0, string list) Belt.Result.t) ->
        Json.t ->
      ...
      fun sourceTransformer ->
        fun constructor ->
          ...
          | _ -> ((Belt.Result.Error (["Expected an array"]))
              [@explicit_arity ])
    and deserialize_SharedTypes__SimpleType__declaration : type arg0.
      (Json.t -> (arg0, string list) Belt.Result.t) ->
        Json.t ->
      ...
      fun sourceTransformer ->
        fun record ->
          ...
          | _ -> ((Belt.Result.Error (["Expected an object"]))
              [@explicit_arity ])
    and deserialize_SharedTypes__SimpleType__expr : type arg0.
      (Json.t -> (arg0, string list) Belt.Result.t) ->
        Json.t ->
      ...
      (fun aTransformer ->
         fun bTransformer ->
           TransformHelpers.deserialize_Stdlib__hashtbl____t aTransformer
             bTransformer : (Json.t -> (arg0, string list) Belt.Result.t) ->
                              (Json.t -> (arg1, string list) Belt.Result.t)
                                ->
                                Json.t ->
                                  ((arg0, arg1) _Stdlib__hashtbl__t,
                                    string list) Belt.Result.t)
            ...
            let inner attr_pos_cnum =
              let inner attr_pos_bol =
                let inner attr_pos_lnum =
                  ...
                  match Belt.List.getAssoc items "pos_fname" (=) with
                  ...
                  | ((Some (json))[@explicit_arity ]) ->
                      (match (fun string ->
                       ...
                       | ((Belt.Result.Error (error))[@explicit_arity ]) ->
                           ((Belt.Result.Error
                               (("attribute pos_fname" :: error)))
                           [@explicit_arity ])
                       | ((Belt.Result.Ok (data))[@explicit_arity ]) ->
                           inner data) in
                match Belt.List.getAssoc items "pos_lnum" (=) with
            ...
            let inner attr_args =
              let inner attr_name =
                let inner attr_path =
                  ...
                  match Belt.List.getAssoc items "module" (=) with
                  ...
                  | ((Some (json))[@explicit_arity ]) ->
                      (match (fun string ->
                       ...
                       | ((Belt.Result.Error (error))[@explicit_arity ]) ->
                           ((Belt.Result.Error
                               (("attribute module" :: error)))
                           [@explicit_arity ])
                       | ((Belt.Result.Ok (data))[@explicit_arity ]) ->
                           inner data) in
                match Belt.List.getAssoc items "path" (=) with
    ...
    and (deserialize_TypeMapSerde__Config____t :
      ...
      fun record ->
              ...
              let inner attr_entries =
                let inner attr_engine =
                  let inner attr_output =
                    ...
                    match Belt.List.getAssoc items "version" (=) with
                    ...
                    | ((Some (json))[@explicit_arity ]) ->
                        (match (fun number ->
                         ...
                         | ((Belt.Result.Error (error))[@explicit_arity ]) ->
                             ((Belt.Result.Error
                                 (("attribute version" :: error)))
                             [@explicit_arity ])
                         | ((Belt.Result.Ok (data))[@explicit_arity ]) ->
                             inner data) in
                  match Belt.List.getAssoc items "output" (=) with
                ...
                | None -> ((Belt.Result.Error (["No attribute engine"]))
                    [@explicit_arity ])
                | ((Some (json))[@explicit_arity ]) ->
                    (match deserialize_TypeMapSerde__Config____engine json
                     with
                     | ((Belt.Result.Error (error))[@explicit_arity ]) ->
        ...
        | _ -> ((Belt.Result.Error (["Expected an object"]))
            [@explicit_arity ])
    and deserialize_TypeMapSerde__Config__Locked__lockedConfig : type arg0.
      (Json.t -> (arg0, string list) Belt.Result.t) ->
        Json.t ->
                ...
                let inner attr_engineVersion =
                  ...
                  | ((Some (json))[@explicit_arity ]) ->
                      (match (fun list ->
                       ...
                       | ((Belt.Result.Ok (data))[@explicit_arity ]) ->
                           inner data) in
                match Belt.List.getAssoc items "engineVersion" (=) with
                | None ->
                    ((Belt.Result.Error (["No attribute engineVersion"]))
                    [@explicit_arity ])
                | ((Some (json))[@explicit_arity ]) ->
                    (match (fun number ->
                              match number with
    ...
    and (deserialize_TypeMapSerde__Config__Locked__lockedEntry :
      ...
      fun record ->
        ...
        | _ -> ((Belt.Result.Error (["Expected an object"]))
            [@explicit_arity ])
    and deserialize_TypeMapSerde__Config__Locked__lockfile : type arg0.
      (Json.t -> (arg0, string list) Belt.Result.t) ->
        Json.t ->
        ...
        fun record ->
          ...
          | ((Json.Object (items))[@explicit_arity ]) ->
              let inner attr_versions =
                let inner attr_engine =
                  Belt.Result.Ok
                    { engine = attr_engine; versions = attr_versions } in
                match Belt.List.getAssoc items "engine" (=) with
                | None -> ((Belt.Result.Error (["No attribute engine"]))
                    [@explicit_arity ])
                | ((Some (json))[@explicit_arity ]) ->
    ...
    and (deserialize_TypeMap__DigTypes____shortReference :
      ...
      fun value ->
        (fun json ->
           ...
           | _ -> ((Belt.Result.Error (["Expected array"]))
               [@explicit_arity ])) value
    and deserialize_TypeMap__DigTypes____typeMap : type arg0.
      (Json.t -> (arg0, string list) Belt.Result.t) ->
        Json.t ->
        ...
        fun value ->
          (deserialize_Stdlib__hashtbl____t
             ...
             (fun json ->
                ...
                | _ -> ((Belt.Result.Error (["Expected array"]))
                    [@explicit_arity ]))) value
    and deserialize_TypeMap__DigTypes____typeSource : type arg0.
      (Json.t -> (arg0, string list) Belt.Result.t) ->
        Json.t ->
...
and deserializeSimpleExpr data =
  ...
  | ((Ok (version, data))[@implicit_arity ]) ->
      (match version with
       | 1 ->
           (match Version1.deserialize_TypeMapSerde__Config____simpleExpr
                    data
            with
            | ((Belt.Result.Error (error))[@explicit_arity ]) ->
                ((Belt.Result.Error (error))[@explicit_arity ])
