...
let rec outputDeclaration = (showSource, declaration) => {
  switch (declaration.body) {
    ...
    | Record(items) => Parsetree.Ptype_record(
    items->List.map(((name, v)) => {Parsetree.pld_name: Location.mknoloc(name),
    pld_mutable: Asttypes.Immutable,
    pld_type: outputExpr(showSource, v)
    }
    )
    )
    | Variant(items) => " = " ++ String.concat("|\n",
    items -> List.map(((name, contents, result)) => {
      name ++ switch (contents) {
        | [] => ""
        | _ => " " ++ String.concat(" * ", contents->List.map(outputExpr(showSource))) ++ switch result {
          | None => ""
...
and outputExpr = (showSource, expr) => {
  switch expr {
    ...
    | Fn(args, result) => String.concat(" -> ", args->List.map(((label, arg)) => switch label {
      | None => ""
      | Some(label) => "~" ++ label
    } ++ outputExpr(showSource, arg))) ++ "->" ++ outputExpr(showSource, result)
