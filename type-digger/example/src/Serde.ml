...
module Version2 =
  struct
    ...
    let rec upgrade_Types__Current__household =
      (fun _input_data ->
         match let rec loop items =
                 match items with
                 | [] -> ((Belt.Result.Ok ([]))[@explicit_arity ])
                 | data::rest ->
                     (match upgrade_Types__Current__person data with
                      | ((Belt.Result.Error (msg))[@explicit_arity ]) ->
                          ((Belt.Result.Error (msg))[@explicit_arity ])
                      | ((Ok (newValue))[@explicit_arity ]) ->
                          (match loop rest with
                           | ((Belt.Result.Error (msg))[@explicit_arity ]) ->
                               ((Belt.Result.Error (msg))[@explicit_arity ])
                           | ((Ok (rest))[@explicit_arity ]) ->
                               ((Ok ((newValue :: rest)))[@explicit_arity ]))) in
               loop _input_data.people
         with
         | ((Belt.Result.Error (err))[@explicit_arity ]) ->
             ((Belt.Result.Error (err))[@explicit_arity ])
         | ((Ok (_converted_people))[@explicit_arity ]) ->
             (match let rec loop items =
                      match items with
                      | [] -> ((Belt.Result.Ok ([]))[@explicit_arity ])
                      | data::rest ->
                          (match upgrade_Types__Current__pet data with
                           | ((Belt.Result.Error (msg))[@explicit_arity ]) ->
                               ((Belt.Result.Error (msg))[@explicit_arity ])
                           | ((Ok (newValue))[@explicit_arity ]) ->
                               (match loop rest with
                                ...
                                | ((Ok (rest))[@explicit_arity ]) ->
                                    ((Ok ((newValue :: rest)))
                                    [@explicit_arity ]))) in
                    loop _input_data.pets
              with
              | ((Belt.Result.Error (err))[@explicit_arity ]) ->
                  ((Belt.Result.Error (err))[@explicit_arity ])
              | ((Ok (_converted_pets))[@explicit_arity ]) ->
                  ((Belt.Result.Ok
                      ({ pets = _converted_pets; people = _converted_people }))
                  [@explicit_arity ])) : Version1._Types__Current__household
                                           ->
                                           (_Types__Current__household,
                                             string) Belt.Result.t)
    and upgrade_Types__Current__person =
      (fun person ->
         ((Ok ({ name = (person.name); age = (float_of_int person.age) }))
         [@explicit_arity ]) : Version1._Types__Current__person ->
                                 (_Types__Current__person, string)
                                   Belt.Result.t)
    and upgrade_Types__Current__pet =
      (fun _input_data -> ((Ok (_input_data))[@explicit_arity ]) : Version1._Types__Current__pet
                                                                    ->
...
let serializeHousehold data =
  wrapWithVersion currentVersion
    (Version2.serialize_Types__Current__household data)
