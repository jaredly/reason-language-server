...
let rec stringify = (t) =>
  ...
  | Object(items) =>
    ...
    ++ String.concat(
         ", ",
         items->List.map(((k, v)) => "\"" ++ String.escaped(k) ++ "\": " ++ stringify(v))
       )
    ++ "}"
  | True => "true"
  | False => "false"
...
let rec stringifyPretty = (~indent=0, t) =>
  ...
  | Array(items) => "[\n" ++ white(indent) ++ String.concat(",\n" ++ white(indent), items->List.map(stringifyPretty(~indent=indent + 2))) ++ "\n" ++ white(indent - 2) ++ "]"
  | Object([]) => "{}"
  | Object(items) =>
    "{\n" ++ white(indent)
    ++ String.concat(
         ",\n" ++ white(indent),
         items->List.map(((k, v)) => "\"" ++ String.escaped(k) ++ "\": " ++ stringifyPretty(~indent=indent + 2, v))
       )
    ++ "\n" ++ white(indent - 2) ++ "}"
  | True => "true"
  | False => "false"
...
module Parser = {
  ...
  let utf8encode = (s) => {
    ...
    let rec ienc = (k, sofar, resid) => {
      ...
      if (resid < 1 lsl bct) {
        s1(prefs->Array.getExn(k) + resid) ++ sofar;
      } else {
