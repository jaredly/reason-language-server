...
module Show = {
  ...
  let state = ({rootPath}, {localModules, compilerPath, dependencyModules}) =>
    ...
    ++ "\nLocal\n"
    ++ (
      Belt.List.map(localModules, ((name, (cmt, src))) =>
        Printf.sprintf("%s (%s : %s)", name, cmt, src)
      )
      |> String.concat("\n")
    )
    ++ "\nDeps\n"
    ++ (
      Belt.List.map(dependencyModules, ((modpath, (cmt, src))) =>
        Printf.sprintf("%s (%s : %s)", _modPath(modpath), cmt, src |? "")
      )
      |> String.concat("\n")
    );
};
...
let newBsPackage = rootPath => {
  ...
  let localModules =
    ...
    |> List.map(((full, rel)) => (FindFiles.getName(rel), (full, rel)));
  ...
  Log.log("Depedency dirs " ++ String.concat(" ", dependencyDirectories));
  {
    ...
    compilationFlags:
      ...
      |> String.concat(" "),
    includeDirectories:
      stdLibDirectories @ dependencyDirectories @ localCompiledDirs,
    compilerPath,
    refmtPath,
  };
};
...
let newJbuilderPackage = rootPath => {
  ...
  let%try ocamllib =
    BuildSystem.getLine("esy sh -c 'echo $OCAMLLIB'", buildDir);
  ...
  let compiledBase =
    ...
    | `Library(libraryName) =>
      buildDir /+ "default" /+ rel /+ "." ++ libraryName ++ ".objs"
    | `Executable(execName) =>
      buildDir /+ "default" /+ rel /+ "." ++ execName ++ ".eobjs"
    };
  let libraryName =
    switch (packageName) {
    | `Library(n) => Some(n)
  ...
  let (otherDirectories, otherFiles) =
    ...
    |> optMap(name => {
         ...
         let res = {
           ...
           let jbuildConfig = JbuildFile.parse(jbuildRaw);
           let%try libraryName =
             JbuildFile.findName(jbuildConfig)
             |> (
               n =>
                 switch (n) {
           ...
           Ok((
             ...
             FindFiles.collectFiles(
               ...
               ~sourceDirectory=otherPath,
               compiledBase,
             )
             |> List.map(((name, p)) => (FindFiles.Plain(name), p)),
           ));
         };
         switch (res) {
         | Error(message) =>
  ...
  let dependencyModules =
    ...
    |> List.map(path => {
         ...
         |> List.map(name => {
              ...
              (
                FindFiles.Plain(
                  Filename.chop_extension(name) |> String.capitalize,
                ),
                (compiled, Some(path /+ name)),
              );
            });
       })
    |> List.concat;
  ...
  |?< (
    libraryName =>
      Hashtbl.replace(
        pathsForModule,
        libraryName ++ "__",
        (compiledBase /+ libraryName ++ "__.cmt", None),
      )
  );
  ...
  Ok({
    ...
    includeDirectories:
      [compiledBase, ...otherDirectories] @ dependencyDirectories,
    compilerPath,
    refmtPath,
  });
};
...
let getPackage = (uri, state) =>
  if (Hashtbl.mem(state.rootForUri, uri)) {
    ...
    let%try root =
      ...
      |> Result.orError("No root directory found");
    switch (root) {
    | `Root(rootPath) =>
...
let newDocsForCmt = (cmtCache, changed, cmt, src, clientNeedsPlainText) => {
  ...
  | None =>
    Log.log("Docs.forCmt gave me nothing " ++ cmt);
    None;
  | Some(docs) =>
...
let newDocsForCmi = (cmiCache, changed, cmi, src, clientNeedsPlainText) => {
  ...
  | None =>
    Log.log("Docs.forCmi gave me nothing " ++ cmi);
    None;
  | Some(docs) =>
...
let docsForCmt = (cmt, src, state) =>
  if (Filename.check_suffix(cmt, ".cmi")) {
    if (Hashtbl.mem(state.cmiCache, cmt)) {
      ...
      | None =>
        Log.log(
          "�\154��\143 cannot get docs for nonexistant cmt " ++ cmt,
        );
        None;
      | Some(changed) =>
        if (changed > mtime) {
...
let getDefinitionData = (uri, state, ~package) =>
  switch (getCompilationResult(uri, state, ~package)) {
  | Success(_, _, data)
  | TypeError(_, _, data) => Some(data)
  };
...
let rec resolveAlias = (state, path, children, ~package) => {
  let rec loop = (path, items) =>
    ...
    | Path.Pident({stamp: 0, name}) =>
      let%opt ((_docs, contents), uri) =
        docsForModule(name, state, ~package);
...
let resolveDefinition = (uri, state, ~package, moduleData, defn) =>
  ...
  | `Local(_, loc, item, docs, _) =>
    Some((
      item |> Definition.docsItem(_, moduleData),
      Some(loc),
      docs,
      Some(uri),
    ))
  | `Global(top, children, suffix) =>
    ...
    | Some(((cmtInfos, data), uri)) =>
      ...
      if (children == []) {
        Some((
          Docs.Module([]),
          Some(topLocation(uri)),
          data.toplevelDocs,
          Some(uri),
        ));
      } else {
        Definition.resolveNamedPath(data, children, suffix)
        |?> (
          ((_, loc, defn, docs)) =>
            Some((
              defn |> Definition.docsItem(_, moduleData),
              Some(loc),
              docs,
              Some(uri),
            ))
        ...
        ((cmt, src)) => {
          ...
          if (children == []) {
            ...
            Some((
              Docs.Module([]),
              uri |?>> topLocation,
              docsForCmt(cmt, src, state) |?> fst,
              uri,
            ));
          } else {
            let%opt (_, contents) = docsForCmt(cmt, src, state);
            let%opt (srcPath, contents, last) =
              Docs.resolveDocsPath(
                ~resolveAlias=resolveAlias(state, ~package),
                uri,
                children,
                contents,
              );
...
let getResolvedDefinition = (uri, defn, data, state, ~package) =>
  Definition.findDefinition(
    defn,
    data,
    resolveDefinition(uri, state, ~package, data),
  );
...
let referencesForPos = (uri, pos, data, state, ~package) => {
  ...
  let externals =
    {
      let%opt_wrap (exportedName, suffixName) =
        Definition.isStampExported(stamp, data);
      ...
      optMap(
        ((modname, (cmt, src))) =>
          if (modname == thisModName) {
            ...
            let%opt data =
              getDefinitionData(Utils.toUri(src), state, ~package);
            let%opt uses =
              Definition.maybeFound(
                Hashtbl.find(data.Definition.externalReferences),
                thisModName,
              );
            let realUses =
              Utils.filterMap(
                ((path, loc, suffix)) =>
                  if (path == [exportedName] && suffix == suffixName) {
                    Some((
                      ...
                      Utils.endOfLocation(
                        loc,
                        String.length(suffixName |? exportedName),
                      ),
                    ));
                  } else {
                    None;
