  ...
  module F = (Collector: {let data: moduleData;}) => {
        ...
        | Tstr_value(_rec, bindings) =>
          ...
          |> List.iter(binding =>
              ...
              | {vb_attributes, vb_pat: {pat_type, pat_desc: Tpat_var({stamp, name}, {loc})}} => {
                ...
                addStamp
                  (stamp, name, loc, Value(pat_type), docs)
              }
    ...
    let enter_pattern = pat =>
      ...
      | Tpat_construct({txt, loc}, {cstr_name, cstr_loc, cstr_res}, args) => {
        ...
        | Tconstr(path, args, _) => addLocation(loc, pat.pat_type, Constructor(path, cstr_name, cstr_loc))
        | _ => ()
        }
      }
    ...
    let enter_expression = expr =>
      ...
      | Texp_constant(_) => addLocation(expr.exp_loc, expr.exp_type, IsDefinition)
      ...
      | Texp_construct({txt, loc}, {cstr_name, cstr_loc, cstr_res}, args) =>
        ...
        switch (dig(cstr_res).Types.desc) {
        | Tconstr(path, args, _) => addLocation(loc, expr.exp_type, Constructor(path, cstr_name, cstr_loc))
        | _ => ()
        }
      | _ => ()
      };
...
let resolveDefinition = (defn, data) => {
  /* Log.log("😍 resolving a definition"); */
  switch defn {
  | IsDefinition => {Log.log("Is a definition"); None}
  ...
  | Path(path) => {
    ...
    | Path.Pident({stamp, name}) => switch (Hashtbl.find(data.stamps, stamp)) {
      | exception Not_found => None
      | (name, loc, item, docs) => Some((name, loc, item, docs))
      }
    | _ => {
      /* Log.log("Not a pident"); */
      None
    }
    }
  }
...
let definitionForPos = (pos, data) => locationAtPos(pos, data) |?> ((_, _, defn)) => resolveDefinition(defn, data);